...
### Production run notes (normative)

- **Invalid snapshot cohorts remain in the store** as `SNAPSHOT_INVALID` Pair Observation Records and Snapshot Summary Records (Evaluation Freeze). They are rendered in a separate `invalid_cohorts` section and are excluded from the calibration gate, from normal profile metrics, and from matured-cohort counts. `UNKNOWN` is not a valid ranking profile and never produces a calibration-gate row.
- **Top-N metrics** for N ∈ {5, 10, 20, 30} are the arithmetic mean of `realized_return` over `OUTCOME_AVAILABLE` cohort members whose `rank_at_selection` is ≤ N. Members with other terminal states are ignored; if no member in the cut is available, the metric is `null` and `top_N_available_count` is `0`. All four cuts use the same helper/policy.
- **Legacy immutable summaries** are never re-evaluated or overwritten. If a persisted summary lacks `top_N_available_count` fields, the report renders them as `null` and does not recompute them from observation records. This is the expected state for pre-fix summaries such as 2026-07-22.
- **Pre-fix persisted summary artifact freeze:** Pre-fix persisted summary artifacts (e.g., the 2026-07-22 summary) are immutable and do not contain `top_N_available_count` fields. Do not rerun `hunter outcome evaluate` on the same cohort/profile/horizon with the new code; the new summary would differ from the existing immutable artifact and `EvaluationStoreError` will fail closed. This is expected Evaluation Freeze behavior, not a migration trigger. The new Top-N policy applies only to new terminal summaries produced after the fix; legacy summaries are consumed by the report layer in a schema-tolerant manner and missing count fields are rendered as `null`.
