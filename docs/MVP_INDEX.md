...
- `docs/planning/SPEC-077-DRAFT.md` exists for MVP-77 Hunter Doctor, Versioning and Controlled Update Framework. Complete; tagged `v0.77.0-dev` at `cddaa87` (local-only; no push). Adds `src/hunter/core/doctor/` with `hunter doctor`, `hunter update check`, and `hunter update plan` (no `apply` in Phase A); read-only Git; deterministic non-executing update plan; rsync/exported deployments supported as a valid doctor runtime.
...
| MVP-76 | Ranking Outcome Evaluation and Calibration (Phase A) | `SPEC-076` | `v0.76.0-dev` | `research_outcome_evaluation` | `test_research_outcome_evaluation` | approved and committed — tag pending | Committed `ce32a12` (local-only; no push). Selected-only evaluation of immutable JSON snapshot audit artifacts against local 1h Feather prices; mandated terminal-state resolution; descriptive-only metrics (Top-5/10/20/30, full-cohort Spearman, benchmark-relative, MAE/MFE, realized volatility); turnover/retention versus `D_prev`; append-only atomic store; CLI (`outcome evaluate`/`outcome report`) with distinct-directory validation, calibration-gate reporting, finite coverage checks, ISO date parsing, and fail-closed persisted-date handling. Independent review and Chimera Critic verdict APPROVE (0 Critical / 0 High). 164 focused tests; full suite 10,680 passed, 3 skipped. |
| MVP-77 | Hunter Doctor, Versioning and Controlled Update Framework | `SPEC-077` | `v0.77.0-dev` | `hunter/core/doctor` (`doctor.py`, `update.py`, `config.py`, `version.py`, `gitutil.py`, `report.py`, `cli.py`, `checks/`) | `test_doctor` | tagged | Planning draft at `docs/planning/SPEC-077-DRAFT.md`; framework committed (local-only; no push). `hunter doctor` (9 categories, exit 0/1/2), `hunter update check` (read-only `git ls-remote`, `--offline`, graceful `UNKNOWN`), `hunter update plan` (deterministic non-executing plan, no `apply`). Shared config resolution with provenance. Argument-aware read-only Git allowlist. Supports rsync/exported deployments without `.git` as valid doctor runtime; `update plan` requires a Git clone. 89 focused tests; full suite 10,781 passed, 3 skipped. Tagged `v0.77.0-dev` at `cddaa87` (local-only; no push). |
