...
| MVP-77 | Hunter Doctor, Versioning and Controlled Update Framework | `SPEC-077` | `v0.77.0-dev` | `hunter/core/doctor` (`doctor.py`, `update.py`, `config.py`, `version.py`, `gitutil.py`, `report.py`, `cli.py`, `checks/`) | `test_doctor` | approved — tag pending | Planning draft at `docs/planning/SPEC-077-DRAFT.md`; framework committed (local-only; no push). `hunter doctor` (9 categories, exit 0/1/2), `hunter update check` (read-only `git ls-remote`, `--offline`, graceful `UNKNOWN`), `hunter update plan` (deterministic non-executing plan, no `apply`). Shared config resolution with provenance. Argument-aware read-only Git allowlist. Supports rsync/exported deployments without `.git` as valid doctor runtime; `update plan` requires a Git clone. 89 focused tests; full suite 10,781 passed, 3 skipped. |
