...
- **Tests:** 149 tests in `tests/test_pairlist_export/` (up from 75; 74 new, 0 removed) covering discovery/containment/duplicate-source, schema/timestamp/value validation, windowing, liquidity (incl. exact tied-liquidity percentile), data quality, verified RS-engine integration, all three ranking profiles and every `PROFILE_FIELD_MISMATCH` rule, deterministic JSON/fingerprint (byte-identical reruns), CLI end-to-end (`feather-input`, `from-feather`, single-pair `BELOW_MIN_PAIRS`), and one real-external-Feather-fixture acceptance test. Full suite: 10,418 passed, 2 skipped (up from 10,344; no regressions). `py_compile` clean; source scan for network/exchange terms clean.
- No version/tag change. No commit. No push.
