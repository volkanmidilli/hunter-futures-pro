...
- **New: read-only Feather-to-ranking-input adapter.** `hunter pairlist feather-input`/`from-feather` convert local Freqtrade `BASE_USDT_USDT-1h-futures.feather` OHLCV files directly into a `ranking-input.json` v2 artifact (and, for `from-feather`, straight through the existing SPEC-074 rank/gate/publish/snapshot pipeline) — closing the "producing ranking-input.json from live reports is a separate, not-yet-built glue step" gap noted in `docs/research/pairlist_export.md` for the local-Feather source.
  - `feather_models.py` / `feather_adapter.py` — safe non-recursive discovery (`^[A-Z0-9]+_USDT_USDT-1h-futures\.feather$`, symlink/hidden/temp/duplicate-source rejection, path containment), schema/timestamp/value validation (duplicate/out-of-order timestamps, future candles, non-finite/invalid close or volume), `[as_of_date-90d, as_of_date)` windowing, and deterministic liquidity (`close×volume → daily total → 30-day average → log1p → cross-sectional average-rank percentile`) and data-quality (expected-1h-slot-coverage) scoring.
  - **Reuses `relative_strength.build_relative_strength_report` unmodified** for the RS dimension (via a daily-close resampling bridge) — no relative-strength algorithm is reimplemented.
  - `ranking_input_v2.py` — new `schema_version: hunter-ranking-input-v2` contract, three ranking profiles (`V1_RS_OI`, `V2_RS_LIQUIDITY`, `V2_RS_OI_LIQUIDITY`) each with their own tie-break order and required-evidence rule, and `PROFILE_FIELD_MISMATCH` validation (a mismatched payload is rejected, never silently reinterpreted).
  - `ranking_adapter.rank_pairs_v2`, `validator.run_publish_gate_v2`, `audit.build_audit_record_v2`, `fingerprint.compute_pair_fingerprint_v2`/`compute_audit_fingerprint_v2` — new v2-profile-aware functions added *alongside* (never modifying) the existing v1 `rank_pairs`/`run_publish_gate`/`build_audit_record`/`compute_pair_fingerprint`/`compute_audit_fingerprint`. A missing `schema_version` always resolves to v1; every pre-existing SPEC-074 test and behavior is unchanged (verified: the original 75 `tests/test_pairlist_export/` tests pass byte-for-byte identically before and after this change).
  - Volume is never represented as open interest: `oi_scores` is always `{}` and `oi_available` is always `false` under `V2_RS_LIQUIDITY`; genuine OI remains a separate, not-yet-built adapter.
  - New dependencies: `pandas>=2.0.0`, `pyarrow>=14.0.0` (verified installed: pandas 3.0.3, pyarrow 24.0.0) — the only way to read Feather files; no other new dependency was added.
- **CLI:** `hunter pairlist feather-input --data-dir <dir> --output <path> --as-of <date>` and `hunter pairlist from-feather --data-dir <dir> --output-dir <dir> [--snapshot-dir <dir>] --as-of <date>`, both routed through the existing `hunter.core.cli`/`pairlist_export.cli` dispatch, listed in the unified `hunter --help` output and `docs/reference/CLI_REFERENCE.md`.
- **Safety:** read-only discovery and validation only — no network, download, trading, scheduler, server, queue, or database access; source Feather files are opened for reading only and are never modified (verified via SHA-256 before/after in tests, including against a real Freqtrade-produced fixture); no `data/`/`reports/` repository access; same fail-closed `PairlistExportSafetyFlags` invariants as SPEC-074.
- **M1 remediation (data-quality contract hardening):**
  - `RankedPair` now carries `data_quality_pct: Decimal | None = None` (backward-compatible additive field; v1 `rank_pairs` behavior unchanged).
  - New reason code `PROFILE_EVIDENCE_INCOMPLETE` for missing required `data_quality` under v2 profiles; `PROFILE_FIELD_MISMATCH` retained for malformed, non-finite, or out-of-range (`<0`, `>100`) values.
  - `validate_profile_fields` rejects missing v2 `data_quality` (`ProfileEvidenceIncompleteError`) and non-Decimal/malformed values (`ProfileFieldMismatchError`); boundaries 0 and 100 accepted.
  - `rank_pairs_v2` populates `data_quality_pct` on every `RankedPair` and propagates profile-validation errors.
  - `run_publish_gate_v2` independently revalidates every selected pair's `data_quality_pct` — it does not rely on `reason_codes` alone; externally constructed invalid rankings are rejected.
  - `PROFILE_ACTIVE_DIMENSIONS` corrected: `V2_RS_LIQUIDITY` is `("rs", "liquidity", "data_quality")`; `V2_RS_OI_LIQUIDITY` is `("rs", "oi", "liquidity", "data_quality")`; v1 unchanged. `ignored_score_dimensions` remains empty.
  - Placeholder M1 tests replaced with 61 real production-path assertions in `tests/test_pairlist_export/test_data_quality_spec075_m1.py` covering validation, ranking, gate revalidation, boundaries, and exact audit dimension ordering.
- **Tests:** 211 tests in `tests/test_pairlist_export/` (up from 149; 62 new, 0 removed) covering discovery/containment/duplicate-source, schema/timestamp/value validation, windowing, liquidity (incl. exact tied-liquidity percentile), data quality, verified RS-engine integration, all three ranking profiles and every `PROFILE_FIELD_MISMATCH`/`PROFILE_EVIDENCE_INCOMPLETE` rule, deterministic JSON/fingerprint (byte-identical reruns), CLI end-to-end (`feather-input`, `from-feather`, single-pair `BELOW_MIN_PAIRS`), publish-gate data-quality revalidation, and one real-external-Feather-fixture acceptance test. Full suite: 10,480 passed, 2 skipped (up from 10,418; no regressions). `py_compile` clean; source scan for network/exchange terms clean.
- **Docs:** architecture, operations, technical, CLI reference, and current-state documents updated for the SPEC-075 Feather flow, v2 audit contract, and M1 remediation.
- Broad multi-pair production Feather acceptance against a real server data root has **not** been executed as of this change; it is tracked in `docs/operations/RELEASE_CHECKLIST.md` and must run before any release tag.
- No version/tag change. No push.
